...
model = gensim.models.doc2vec.Doc2Vec(size=50, min_count=2, iter=700)
...
result.write("<h1><font color=\"RED\"> Similarity:%s </font></h1>"%sims[index+1][1])
...
result.write("<tr><td> %s </td>"%(' '.join(train_corpus[0].words)))
print(u'Similirity : %s : «%s»\n' % (sims[1], ' '.join(train_corpus[0].words)))
result.write("<td>%s:</td>"%' '.join(train_corpus[1].words))
